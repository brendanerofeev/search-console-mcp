...
export async function login() {
    ...
    log(`   Submitting or deleting sitemaps, and adding or removing properties,`);
    log(`   require the read-write ${colors.cyan}webmasters${colors.reset} scope. Without it those tools are`);
    log(`   still listed but fail with "insufficient authentication scopes".`);
    log(`   Decline to keep read-only access, which is enough for all reporting.`);

    const useWrites = await prompts.confirm(
        'Authorize the Search Console write scope (sitemaps, properties)?',
        false
    );
