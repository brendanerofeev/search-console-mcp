 ...
 *
 * Why this is a factory and not a module-level singleton: an `McpServer` wraps
 * one Protocol instance, which binds to exactly ONE transport and throws
 * "Already connected to a transport" on a second connect(). The MCP SDK is
 * explicit that you should "use a separate Protocol instance per connection",
 * and its streamable-HTTP transport enforces the same rule from the other side
 * (a stateless transport refuses to be reused across requests, and a stateful
 * one refuses a second initialize).
 *
 * Sharing one server therefore capped the process at a single concurrent
 * client — and, before this was handled, crashed the whole process when a
 * second one connected. Registration is pure (tool definitions and closures
 * over module-level helpers), so building a server per connection is cheap and
...
// Single instance for the stdio transport, which is inherently one-client.
...
async function main() {
  ...
  if (isSseMode) {
    ...
    // Pass the factory, not the instance: the HTTP transport builds one server
    // per client session so several agents can work concurrently.
    await startSseServer(createMcpServer, port);
  } else {
